...
4. ~~방찾기/빠른매칭~~ ✅ — 시작하기 한 번으로 조건(인원·입장료) 맞는 대기방 랜덤 배정/생성,
   10초 무입장 시 위장 봇(Hard, isBot=false) 1~10초 랜덤 간격 충원, 정원 도달 5초 카운트다운 자동 시작.
   매칭 레이스(봇으로 찬 방)는 점유 필터+봇 방출+클라 자동 재매칭으로 해소.
   **100명 랜덤 폭풍 테스트**(scripts/quickmatch-storm.py): 실플레이 세트 완주 + 지갑/정원/에러
   불변식 검증 — 로컬·fly 각각 100/100 완주, 이상 0건 (2026-08-09).
...
7. ~~머지/배포~~ ✅ — main 머지(7c1df82) + fly/Pages 배포 완료(2026-08-09). 맞춤게임 라이브.

### 백로그 (급하지 않음)
- ~~봇 스톱 성향 과함~~ ✅ 확률 밴드 튜닝(2026-08-09, tools/BotSim으로 실측):
  손합별 스톱 확률(≤2: 80~85%, 3~5: 30~40%, 6~10: 8~12%) + Hard는 쌍 보류 70%(두 번 뽕 노림).
  4인 순수 스톱 58→43%(Normal)/62→47%(Hard), 뽕바가지·족보 등 비중 상승.
...
## 모바일 M3 — 기본기 + UI 정리 (2026-08-13 ~ 08-20)

전부 로컬 커밋(배포 동결 ~8/22, `deploy-freeze` 참조). 상세 상태는 `mobile-roadmap.md` 갱신분 참조.

- **M3 블로커 해소** — 계정 영속(자격 저장+자동 로그인, 401 시 재인증), 안드로이드 뒤로가기
  (OnBackInvokedCallback), 세이프에어리어, 스토어 식별자 빌드 시 강제(CliBuild).
- **설정 오버레이** — 소리/진동 토글, 규칙·약관 안내, 게임 중 나가기. 톱니 위치 전 화면 고정.
- **프로필** — 통계(모드×인원별 표+승률 게이지)/게임 기록(등수·상대·정산액) 탭.
- **서버 치명 버그** — 맞춤게임 위장 봇 정산에서 포인트 신규 생성되던 문제 수정(테스트 234개).
  **프로덕션 미반영 — 8/22 배포 시 최우선 + 증식분 잔액 정리 검토.**
- **메뉴 UI 색 통일(08-18)** — 인라인 색 94개 → `UiTheme.cs` 토큰 17개. 골드는 화면당 "선택된
  것" 하나, 상태 문구 amber 폐지, 대비 4.5:1 하한, 비활성 버튼 라벨 동반 감광(UiKit.SetEnabled).
- **카드 모션 안정화(08-18)** — 모바일 60fps 지정, 손패를 내용 변경시에만 재생성(상태 갱신마다
  갈아끼우던 것이 흔들림 끊김·레이아웃 널뜀의 원인), 흔들림 시계 공유+델타 상한.
- **튜토리얼 전면 개편(08-19~20)** — bestFit 제거·42px 고정 2줄 교습 박스(반투명, 실전 좌석
  유지), 이전/다음 버튼 내장. '이전' = 결정적 리플레이 되감기(판 상태까지 복원, 사용자 카드 선택
  재현, 행동 묶음 단위, 불투명 블링크 전환). 자연뽕 바가지를 플레이어블 레슨으로 신설(8레슨).
  봇 턴도 실전처럼 뽑고 버리기, 뽕 타임 1초 호흡.
- 검증: 에뮬레이터 2대 대전 + 화면 녹화 프레임 분석(드로우 연출 회귀, 블링크 밝기 곡선 등).
- 저장소 정리: `.utmp` 안드로이드 빌드 캐시 53개 파일 추적 해제.

## 다음 단계 — 모바일 출시

Phase 5(멀티)까지 끝나고 웹으로 검증을 마쳤습니다. 이후 계획은 `mobile-roadmap.md`에 따로 정리했습니다.
요약하면 M3 모바일 기본기(토큰 영속·뒤로가기·세이프에어리어) → M4 스토어 준비 → M5 운영 안정성 →
M6 수익화 → M7 출시이고, 법무(GRAC 웹보드 분류)는 병렬로 지금 착수합니다.
