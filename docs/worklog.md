...
1. **R7 지갑 동시성 락** (선행 필수) — 에스크로/정산 동시 요청에서 잔액 깨짐 방지.
   advisory lock 또는 SERIALIZABLE로 지갑 갱신 직렬화(considerations.md R7).
2. **판돈 방 생성/입장/정산** (M2 핵심) — 방장이 인원(2~6)+판돈(100~10000) 설정,
   입장 시 잔액 검증+에스크로, 게임(5라운드) 종료 시 1등 몰아주기(공동 1등 균등분배·절사)
   + 방 폭파(rules.md §9-1~9-3). 기존 Room/GameSession 재사용, 방 설정+정산 레이어만 추가.
   이탈 몰수는 봇 대체 시스템에 연결.
3. **클라 맞춤게임 UI** — 로비 "일반 게임" 진입: 방 설정 화면 + 잔액 표시.
   테이블은 단일 틀 그대로(참가자/판돈 표시만 다름).
4. **방찾기/빠른매칭** — 공개 방 목록 + 조건 자동 입장 (2~3 완료 후).
5. **재접속** — 이탈자가 복귀하면 봇 자리 되찾기(친구방 포함, M2와 독립 진행 가능).
