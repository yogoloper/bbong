...
public static class WsEndpoint
{
    ...
    private static async Task Handle(HttpContext context, RoomRegistry registry, IAccountStore accounts)
    {
        ...
        try
        {
            await ReceiveLoopAsync(socket, registry, sink, member);
        }
        finally
        {
    ...
    private static async Task ReceiveLoopAsync(WebSocket socket, RoomRegistry registry, WebSocketSessionSink sink, RoomMember member)
    {
        ...
        while (socket.State == WebSocketState.Open)
        {
            ...
            Route(registry, sink, member, message);
        }
    }

    private static void Route(RoomRegistry registry, WebSocketSessionSink sink, RoomMember member, object message)
    {
